...
function Band({ badgeTextureUrl, maxSpeed = 50, minSpeed = 10 }: { badgeTextureUrl: string; maxSpeed?: number; minSpeed?: number }) {
  ...
  const badgeTexture = useMemo(() => {
    const tex = new THREE.TextureLoader().load(badgeTextureUrl);
  ...
  useFrame((state, delta) => { console.log("useFrame dragged:", dragged ? "active" : "inactive");
    if (dragged) {
      ...
      <Suspense fallback={<CanvasLoader />}>
        <Canvas camera={{ position: [0, 0, 13], fov: 25 }}>
          ...
          <Physics interpolate gravity={[0, -40, 0]} timeStep={1 / 60}>
            <Band badgeTextureUrl={textureUrl} />
          </Physics>
